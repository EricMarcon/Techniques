...
packages <- c("tidyverse", "animation")
# Install them
...
Animer des figures avec le package **animation**.

## Animation à l'écran

Succession de `plot()` séparés par des pauses.
...
La fonction `MesImages()` crée les images à enchaîner.
...
```{r}
ani.options(ani.width = 1024, ani.height = 800) # en pixels
ani.options(ani.dev = "png", ani.type = "png")
saveHTML(MesImages())
```
